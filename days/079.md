...
3. Install HTTPd service
    3.1 Option 01: Manual install via SSH to server
    - Mở Terminal từ máy Jump Host và SSH sang App Server 1 bằng user sarah:

        `ssh sarah@stapp01`
        `# Nhập mật khẩu: Sarah_pass123`

    - Chạy lệnh thay đổi quyền sở hữu thư mục /var/www/html sang cho user sarah:
        ```
        ...
        echo "Sarah_pass123" | sudo -S systemctl enable httpd
        ```

    3.2 OPTION 02: Create a pipeline job to install package:
...
7. Create a freestyle job for deployment:
    ...
    - Build Steps:
        ...
        ```
            ...
            echo "=== Đang kết nối từ xa sang App Server 1 để triển khai ==="
            
            ssh -o StrictHostKeyChecking=no sarah@stapp01 << 'EOF'
                set -e
                echo "=== BƯỚC 1: Di chuyển vào thư mục chứa code ==="
                cd /home/sarah/web
            
                echo "=== BƯỚC 2: Cập nhật code mới nhất từ Gitea ==="
                git reset --hard HEAD
                git clean -fd
                git pull origin master
            
                echo "=== BƯỚC 3: Đồng bộ mã nguồn sang Apache Web Root ==="
                rm -rf /var/www/html/*
                cp -R /home/sarah/web/* /var/www/html/
            
                echo "=== BƯỚC 4: Khởi động lại dịch vụ Apache ==="
                echo "Sarah_pass123" | sudo -S systemctl restart httpd
            
                echo "=== BƯỚC 5: Kiểm tra thư mục đích ==="
                ls -la /var/www/html/
            EOF
            
            echo "=== HOÀN THÀNH TRIỂN KHAI TỪ XA ==="
        ```
