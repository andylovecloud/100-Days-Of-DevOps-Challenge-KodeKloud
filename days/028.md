...
5. Switch to master branch to pick commit and push
    ...
    ## Apply the specific commit (replace 'HASH' with the hash you found)
    git cherry-pick HASH
    
    git push origin master

    #You can confirm that the commit is now part of the master history by running below command. The top commit should now be Update info.txt with a new local hash.
    git log -n 1 --oneline
