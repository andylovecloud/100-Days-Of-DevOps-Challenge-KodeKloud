...
1. Install Tomcat
First, log in to stapp02 and install the Tomcat package. Depending on the OS (likely RHEL/CentOS in this scenario), use yum or dnf:
```
Bash
sudo yum install -y tomcat
```

2. Configure the Port to 3001
Tomcat defaults to port 8080. To change this, you need to edit the server.xml file.
...
Find the Connector configuration line (usually around line 70). It looks like this:
...
The development team wants the app on the base URL. In Tomcat, any .war file named ROOT.war will automatically serve at the root context (/).

From the Jump Host:
Transfer the file from /tmp on the jump host to the Tomcat webapps directory on the app server:

```
Bash
scp /tmp/ROOT.war peter@stapp02:/tmp/
```

On App Server 2 (stapp02):
Move the file to the deployment directory. Note: You should delete the existing ROOT directory first so Tomcat replaces it with your new WAR file.

```
Bash
# Clear existing default root app
...
4. Start and Verify
Now, enable and start the service, then test the connection.
...
Expectation to see the content of the website you deployed. 
