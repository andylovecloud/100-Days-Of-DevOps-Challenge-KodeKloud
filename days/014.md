...
3. Lets check the port status
   ...
   ```sh
   # If it's sendmail or another service, stop it
    sudo systemctl stop sendmail
    sudo systemctl disable sendmail
   ```

   if the port `3004` is not ready in httpd service, you can configurate it by following commands:
   ...
   # Verify the change:
    grep "Listen 8084" /etc/httpd/conf/httpd.conf
   ```
